...
{
    if (color) {
        printf("{ \"separator_block_width\" : 0, \"separator\" : false, \"full_text\" : \"\", "
               "\"icon\" : \"/usr/share/icons/stlarch_icons/%s.xbm\", \"color\" : \"%s\", "
               "\"icon_color\" : \"%s\" }", icon, color, color);
    } else {
        printf("{ \"separator_block_width\" : 0, \"separator\" : false, \"full_text\" : \"\", "
               "\"icon\" : \"/usr/share/icons/stlarch_icons/%s.xbm\""
               " }", icon, color);
    }
...
static void print_text(const char *text, const char *color, int nocomma)
{
    if (color) {
        ...
        printf("{ \"separator\" : false, \"full_text\" : \"%s \" }", text);
    }
